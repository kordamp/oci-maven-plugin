 ...
 */
@CompileStatic
trait OptionalGatewayNameAwareTrait implements PathAware, ExecutionIdAware, LogAware {
    @Parameter(property = 'oci.gateway.name', name = 'gatewayName')
    String gatewayName

    String getGatewayName() {
        stringProperty(this, 'OCI_GATEWAY_NAME', 'oci.gateway.name', this.@gatewayName)
    }

    void validateGatewayName() {
        if (isBlank(getGatewayName())) {
            setGatewayName('gateway-' + UUID.randomUUID().toString())
            log.warn("Missing value for 'gatewayName' in $path. Value set to ${getGatewayName()}")
        }
    }
}
