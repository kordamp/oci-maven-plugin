 ...
 */
@CompileStatic
@Mojo(name = 'list-autonomous-dabases')
class ListAutonomousDatabasesMojo extends AbstractOCIMojo implements CompartmentIdAwareTrait, VerboseAwareTrait {
    @Override
    void executeGoal() {
        ...
        for (AutonomousDatabaseSummary database : response.items) {
            println(database.displayName + (isVerbose() ? ':' : ''))
            if (isVerbose()) {
