 ...
 */
@CompileStatic
trait OptionalDatabaseNameAwareTrait implements PathAware, ExecutionIdAware, LogAware {
    @Parameter(property = 'oci.database.name', name = 'databaseName')
    String databaseName

    String getDatabaseName() {
        stringProperty(this, 'OCI_DATABASE_NAME', 'oci.database.name', this.@databaseName)
    }

    void validateDatabaseName() {
        if (isBlank(getDatabaseName())) {
            setDatabaseName('database-' + UUID.randomUUID().toString())
            log.warn("Missing value for 'databaseName' in $path. Value set to ${getDatabaseName()}")
        }
    }
}
