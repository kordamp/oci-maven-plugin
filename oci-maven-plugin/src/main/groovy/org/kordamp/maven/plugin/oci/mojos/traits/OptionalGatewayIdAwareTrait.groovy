 ...
 */
@CompileStatic
trait OptionalGatewayIdAwareTrait implements PathAware, ExecutionIdAware {
    @Parameter(property = 'oci.gateway.id', name = 'gatewayId')
    String gatewayId
    ...
    void validateGatewayId() {
        if (isNotBlank(getGatewayId()) && !isValid(getGatewayId())) {
