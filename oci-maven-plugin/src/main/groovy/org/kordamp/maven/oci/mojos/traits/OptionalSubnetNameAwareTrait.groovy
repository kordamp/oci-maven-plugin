 ...
 */
@CompileStatic
trait OptionalSubnetNameAwareTrait implements PathAware, LogAware {
    @Parameter(property = 'oci.subnet.name', name = 'subnetName')
    String subnetName

    String getSubnetName() {
        stringProperty('OCI_SUBNET_NAME', 'oci.subnet.name', this.@subnetName)
    }

    void validateSubnetName() {
        if (isBlank(getSubnetName())) {
            setSubnetName('subnet-' + UUID.randomUUID().toString())
            log.warn("Missing value for 'subnetName' in $path. Value set to ${getSubnetName()}")
        }
    }
}
